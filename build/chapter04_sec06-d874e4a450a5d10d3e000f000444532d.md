...
In machine learning, $\tanh$ serves as one of the classical **activation
functions** in neural networks. Each neuron computes a weighted sum of its
inputs and passes the result through an activation function; using $\tanh$
squashes any real-valued input into the range $(-1, 1)$ and introduces the
nonlinearity that allows a network to learn complex patterns. The fact that
$\tanh$ is odd and therefore zero-centered is a practical advantage over the
logistic sigmoid $\sigma(x) = 1/(1 + e^{-x})$, which maps to $(0, 1)$:
zero-centered outputs tend to make gradient-based training more efficient. The
two functions are in fact closely related by $\tanh(x) = 2\sigma(2x) - 1$, so
they represent the same fundamental shape, merely scaled and shifted.
