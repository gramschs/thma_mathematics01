...
And if we have ever used a neural network we have almost certainly encountered
$\tanh$ again. It is one of the go-to **activation functions** that sit inside
each neuron, squashing whatever number comes in into the range $(-1, 1)$. That
boundedness is exactly what we want: without it, values would blow up as they
pass through layer after layer. The zero-centered output is a bonus too, since
it makes training faster compared to the logistic sigmoid $\sigma(x) = 1/(1 +
e^{-x})$, which does the same job but shifted up into $(0, 1)$. The two are
really the same curve under the hood: $\tanh(x) = 2\sigma(2x) - 1$.
