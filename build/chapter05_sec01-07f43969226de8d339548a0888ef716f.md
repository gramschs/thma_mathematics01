...
surface throughout its domain:

\begin{equation*}
h(x) = 0.02\,x^2 - 0.6\,x + 6, \quad x \in [0, 30],
\end{equation*}

where $x$ is the position along the track in centimetres and $h$ is the height
above the table surface in centimetres. The rail starts at $h(0) = 6~\text{cm}$,
descends to its lowest point at $x = 15~\text{cm}$ with $h(15) = 1.5~\text{cm}$,
and then rises symmetrically back to $h(30) = 6~\text{cm}$ at the finishing
block. Unlike the straight rail from section 3.1, this track does not descend
...
\begin{equation*}
h(5) = 0.02 \cdot 25 - 0.6 \cdot 5 + 6 = 0.5 - 3 + 6 = 3.5~\text{cm},
\end{equation*}

\begin{equation*}
h(20) = 0.02 \cdot 400 - 0.6 \cdot 20 + 6 = 8 - 12 + 6 = 2~\text{cm}.
\end{equation*}
...
\begin{equation*}
\frac{h(20) - h(5)}{20 - 5} = \frac{2 - 3.5}{15} = \frac{-1.5}{15} = -0.1~\frac{\text{cm}}{\text{cm}}.
\end{equation*}
...
Here the letter $h$ in the denominator denotes the horizontal step size, not
the height function. This notation is standard, and the context always makes
the meaning clear. The numerator is the change in height, the denominator is
the change in position, and the quotient is the average slope of the track over
the interval $[x_0,\, x_0 + h]$.

This ratio is called the **difference quotient** of the function $h$ at the
...
\begin{equation*}
\frac{h(10) - h(5)}{5} = \frac{(0.02 \cdot 100 - 0.6 \cdot 10 + 6) - 3.5}{5}
...
\begin{equation*}
\frac{h(6) - h(5)}{1} = (0.02 \cdot 36 - 0.6 \cdot 6 + 6) - 3.5
= 3.12 - 3.5 = -0.38.
\end{equation*}
...
<!--
  ...
  An interactive plot of the parabolic marble track h(x) = 0.02x² - 0.6x + 6
  on the domain [0, 30].
  ...
  Display:
    ...
    - Two filled dots at (x0, h(x0)) and (x0+h, h(x0+h)), labelled P1 and P2.
    - A shaded rectangle on the x-axis showing the interval [x0, x0+h],
      labelled Δx = h.
    - A vertical arrow from h(x0) to h(x0+h) at position x0+h,
      labelled Δh = h(x0+h) - h(x0).
    - The secant line through P1 and P2, extended across the full plot.
  
  Info tiles below the chart:
    - "Δx" showing the step size h.
    - "Δh" showing h(x0+h) - h(x0), rounded to 3 decimal places.
    - "Difference quotient Δh/Δx" showing the computed value, rounded to 3
...
we drew in section 3.5. The two points $P_1 = (x_0,\, h(x_0))$ and
$P_2 = (x_0 + h,\, h(x_0 + h))$ both lie on this curve. Draw the straight
...
\begin{equation*}
m = \frac{\Delta h}{\Delta x} = \frac{h(x_0 + h) - h(x_0)}{h}.
\end{equation*}
...
<!--
  ...
  Adapt the function to the marble track: h(x) = 0.02x² - 0.6x + 6,
  domain [0, 30]. Fix x0 = 5, slider range for h: [0.01, 10], default h = 5.
  Analytical derivative: h'(x) = 0.04x - 0.6, so h'(5) = -0.4.
  ...
  The app already shows:
    ...
    - Info tiles: secant slope, approximation error |Δh/Δx - h'(x0)|,
