...
Graph of the second derivative $h''(x) = \dfrac{1}{15} - \dfrac{x}{450}$ on the interval $[0, 60]$, showing the positive values on the lower half of the lift hill, the zero at $x = 30~\text{m}$, and the negative values on the upper half. (Source: own figure; licence [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0))
```
