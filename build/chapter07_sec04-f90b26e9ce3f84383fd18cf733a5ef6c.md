...
```{raw} html
<!--
  OPTIONAL INTERACTIVE IDEA: "Inflection point explorer"
...
We have defined the inflection point as a point where the sign of the second
derivative, and therefore the convexity/concavity of a function, changes. We
identified the necessary condition $f''(x_0) = 0$, and we showed through the
example of $g(x) = x^4$ that this condition is not sufficient on its own. For
the roller coaster height profile $h(x)$, the unique inflection at
$x = 30~\text{m}$ marks the transition from the bowl-shaped lower half of the
lift hill to the dome-shaped upper half, at the point of maximum gradient,
